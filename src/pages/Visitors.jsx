...
export default function Visitors() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/get/${NAMESPACE}/${KEY}`)
      ...
      .catch(() => { setError('Failed to fetch counter.'); setLoading(false) })
  }, [authed])

  // ── Reset counter to 0 ─────────────────────────────────
  const reset = async () => {
    if (!window.confirm('Reset to 0?')) return

    try {
      const res = await fetch(`${API}/set/${NAMESPACE}/${KEY}?value=0`)
    ...
    <div style={s.page}>
      <div style={s.box}>
        ...
        <div style={{ marginTop: 20 }}>
          <button
            onClick={() => { sessionStorage.removeItem('vjp'); setAuthed(false) }}
